...
export default function () {
    const [log, setL] = React.useState(['HELLO'])
    ...
    const handleInput = (i=0) => ({key: i, value: data[i], onChange: e => {
        setD(p => p.map((v, k) => k===i? e.target.value: v))
    }})
    const handleSelect = (i=0) => ({value: state[i][0], onChange: e => {
    ...
    return (
        <div>
          ...
          , [state])}
          {[0, 1, 2].map(i =>
            <input type="number" min="0" max="255" {...handleInput(i)} />
          )}
