...
export function Tut (props, ref) {
    const { note, ...other } = props
    const bind = state => {
        const { noteNum, velocity, send } = state
        if (ref?.current && note === noteNum)
            ref.current.active = velocity > 0
        }
    return <UseNote as="div" ref={ref} {...other} note={bind}/>
}

import styled from 'styled-components'
import { forwardRef } from 'react'

export const Black = styled(forwardRef(Tut))`
  position: absolute;
  top: 0px;
  left: -12px;
  width: 20px;
...
export const White = styled(forwardRef(Tut))`
  display: block;
  height: 220px;
  background: #fff;
